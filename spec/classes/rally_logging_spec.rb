...
describe 'rally::logging' do
  ...
  shared_examples 'basic default logging settings' do
    it 'configures rally logging settins with default values' do
      ...
      is_expected.to contain_rally_config('DEFAULT/log_file').with(:value => '/var/log/rally/rally.log')
      ...
      is_expected.to contain_rally_config('DEFAULT/rally_debug').with(:value => '<SERVICE DEFAULT>')
    end
  end

  shared_examples 'basic non-default logging settings' do
    it 'configures rally logging settins with non-default values' do
      ...
      is_expected.to contain_rally_config('DEFAULT/rally_debug').with(:value => 'true')
    end
  end

  shared_examples_for 'logging params set' do
    it 'enables logging params' do
      is_expected.to contain_rally_config('DEFAULT/logging_context_format_string').with_value(
        '%(asctime)s.%(msecs)03d %(process)d %(levelname)s %(name)s [%(request_id)s %(user_identity)s] %(instance)s%(message)s')

      is_expected.to contain_rally_config('DEFAULT/logging_default_format_string').with_value(
        '%(asctime)s.%(msecs)03d %(process)d %(levelname)s %(name)s [-] %(instance)s%(message)s')

      is_expected.to contain_rally_config('DEFAULT/logging_debug_format_suffix').with_value(
        '%(funcName)s %(pathname)s:%(lineno)d')

      is_expected.to contain_rally_config('DEFAULT/logging_exception_prefix').with_value(
       '%(asctime)s.%(msecs)03d %(process)d TRACE %(name)s %(instance)s')

      is_expected.to contain_rally_config('DEFAULT/log_config_append').with_value(
        '/etc/rally/logging.conf')
      is_expected.to contain_rally_config('DEFAULT/publish_errors').with_value(
        true)

      is_expected.to contain_rally_config('DEFAULT/default_log_levels').with_value(
      ...
      is_expected.to contain_rally_config('DEFAULT/fatal_deprecations').with_value(
        true)

      is_expected.to contain_rally_config('DEFAULT/instance_format').with_value(
        '[instance: %(uuid)s] ')

      is_expected.to contain_rally_config('DEFAULT/instance_uuid_format').with_value(
        '[instance: %(uuid)s] ')

      is_expected.to contain_rally_config('DEFAULT/log_date_format').with_value(
        '%Y-%m-%d %H:%M:%S')
    end
  end
  ...
  shared_examples_for 'logging params unset' do
   [ :logging_context_format_string, :logging_default_format_string,
     ...
     :log_date_format, ].each { |param|
        it { is_expected.to contain_rally_config("DEFAULT/#{param}").with_value('<SERVICE DEFAULT>') }
      }
  end
